...
#    check_opts(opts)
    ...
    f = open(('../MFSD/ORI/testList.txt'), 'r')
    line = f.readline()
...
#        print(idx)
        aa = line.split(' ')
        inImg = aa[0]
        fPath = inImg.replace(inImg.split('/')[-1], '')
#        ifd = fPath.split('/')[-2]
        ifd = fPath
        if(ifd != fdMark):
            inFolder = ifd
            print(inFolder)
            outFolder = inFolder.replace('ORI', 'mfsdPhoto')
            print(outFolder)
            tl.files.exists_or_mkdir(outFolder)

            files = list_files(inFolder)
            full_in = [os.path.join(inFolder,x) for x in files]
            full_out = [os.path.join(outFolder,x) for x in files]
